...
const CheckoutPage: React.FC<CheckoutPageProps> = ({ user, logout }) => {
  ...
  useEffect(() => {
      if (cart.length > 0) {
          // Meta Pixel Tracking (Corrected Payload)
    ...
    try {
        ...
        const options = {
            ...
            handler: async function (response: any) {
                // --- CAPI: Generate unique event ID for deduplication ---
                const event_id = 'purchase_' + Date.now() + '_' + Math.random().toString(36).substring(2);

                // --- Meta Pixel Tracking (Client-Side) ---
                trackEvent('Purchase', {
                    contents: cart.map(item => ({
                        id: item.sku || item.id,
                        quantity: item.quantity,
                    ...
                    currency: 'INR',
                    order_id: order_id,
                    event_id: event_id // Send event ID to Pixel
                });
                ...
                // --- Trigger Backend Order Creation (with Event ID for CAPI) ---
  ...
  const verifyAndPlaceOrder = async (paymentInfo: any, eventId: string) => {
      const orderData = {
        userId: user?.id,
        ...
        userAgent: navigator.userAgent // Pass user agent for CAPI
      };
