        ...
        try {
            ...
            const options = {
                ...
                handler: async (response: any) => {
                    const verifyRes = await fetch(getApiUrl('orders/verify-standard'), {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                    ...
                    <div className="lg:col-span-7 space-y-12 animate-in slide-in-from-left-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <input type="text" placeholder="Recipient name" value={customerInfo.name} onChange={e => setCustomerInfo({...customerInfo, name: e.target.value})} className="w-full rounded-md border border-zinc-200 bg-white p-3 text-sm focus:ring-1 focus:ring-zinc-950 outline-none shadow-sm" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-zinc-500 uppercase ml-1">Mobile Number</label>
                                <input type="tel" placeholder="10-digit number" value={customerInfo.phone} onChange={e => setCustomerInfo({...customerInfo, phone: e.target.value})} className="w-full rounded-md border border-zinc-200 bg-white p-3 text-sm focus:ring-1 focus:ring-zinc-950 outline-none shadow-sm" />
                            </div>
                        </div>

                        <div className="space-y-2">
                            <label className="text-xs font-bold text-zinc-500 uppercase ml-1">Email Address</label>
                            <input type="email" placeholder="name@example.com" value={customerInfo.email} onChange={e => setCustomerInfo({...customerInfo, email: e.target.value})} className="w-full rounded-md border border-zinc-200 bg-white p-3 text-sm focus:ring-1 focus:ring-zinc-950 outline-none shadow-sm" />
                        </div>

                        <div className="space-y-2">
                            <label className="text-xs font-bold text-zinc-500 uppercase ml-1">Street Address</label>
                            <textarea placeholder="Apt, Suite, Street name" value={customerInfo.address} onChange={e => setCustomerInfo({...customerInfo, address: e.target.value})} className="w-full rounded-md border border-zinc-200 bg-white p-3 text-sm focus:ring-1 focus:ring-zinc-950 outline-none shadow-sm resize-none" rows={3} />
                        </div>

                        <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-zinc-500 uppercase ml-1">City</label>
                                <input type="text" placeholder="City" value={customerInfo.city} onChange={e => setCustomerInfo({...customerInfo, city: e.target.value})} className="w-full rounded-md border border-zinc-200 bg-white p-3 text-sm focus:ring-1 focus:ring-zinc-950 outline-none shadow-sm" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-zinc-500 uppercase ml-1">Pincode</label>
                                <input type="text" placeholder="6-digit code" value={customerInfo.postalCode} onChange={e => setCustomerInfo({...customerInfo, postalCode: e.target.value})} className="w-full rounded-md border border-zinc-200 bg-white p-3 text-sm focus:ring-1 focus:ring-zinc-950 outline-none shadow-sm" />
                            </div>
                        </div>
                    </div>
                                ...
                                {cart.map(item => (
                                    <div key={item.id} className="flex justify-between items-center text-sm">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 bg-zinc-50 rounded border border-zinc-100 overflow-hidden">
