...
const ProductStickyBar: React.FC<ProductStickyBarProps> = ({ isVisible, product, selectedVariants, onVariantChange, onAddToCart, quantity }) => {
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-gray-200 shadow-[0_-4px_12px_rgba(0,0,0,0.08)] z-40 transition-transform duration-300 ease-in-out ${
                ...
                <div className="flex items-center gap-4">
                  {product.variants.map((variant) => (
                    <div key={variant.name} className="flex items-center">
                      <label className="text-xs font-medium text-gray-600 mr-2 hidden lg:block">{variant.name}</label>
                      <div className="relative">
                        <select
                          value={selectedVariants[variant.name]}
                          onChange={(e) => onVariantChange(variant.name, e.target.value)}
                          className="appearance-none bg-white border border-gray-300 rounded-md pl-3 pr-8 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-rose-500 focus:border-rose-500 transition-all"
