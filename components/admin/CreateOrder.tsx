...
const CreateOrder: React.FC<CreateOrderProps> = ({ token, onOrderCreated }) => {
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(async () => {
            if (searchTerm.length > 1) {
                const res = await fetch(getApiUrl(`/api/products/search?q=${searchTerm}`));
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(async () => {
            if (customerSearch.length > 1) {
                // Assuming we have a user search endpoint or using basic filter on all users
    ...
    const selectCustomer = (user: User) => {
        setCustomerInfo({
            ...
            name: user.name,
            email: user.email,
            phone: (user as any).phone || '', // Need to ensure phone exists in User type or fetch from orders
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                onOrderCreated(); // Switch view back to list
            } else {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-visible">
                        <div className="p-4 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <button className="text-sm text-blue-600 hover:underline">Add custom item</button>
                        </div>
                        <div className="p-4">
                            {/* Search */}
                            <div className="relative mb-4" ref={searchRef}>
                                <div className="flex items-center border border-gray-300 rounded-md focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500">
                                    ...
                                    />
                                    <button className="px-4 py-2 bg-gray-100 border-l hover:bg-gray-200 text-sm font-medium text-gray-600 rounded-r-md">Browse</button>
                                </div>
                                
                                {isSearching && searchResults.length > 0 && (
                                    <div className="absolute z-50 w-full bg-white border border-gray-200 shadow-xl rounded-md mt-1 max-h-60 overflow-y-auto">
                            ...
                            {/* Line Items */}
                            {lineItems.length === 0 ? (
                                <div className="text-center py-8 text-gray-500 text-sm">No products added.</div>
                            ) : (
                                <div className="space-y-4">
                                    {lineItems.map((item, idx) => (
                                        <div key={idx} className="flex items-start gap-4 py-2 border-b border-gray-50 last:border-0">
                                            ...
                                            <div className="flex items-center">
                                                ...
                                                />
                                                <p className="text-sm font-bold text-gray-800 w-20 text-right">₹{item.price * item.quantity}</p>
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div className="p-4 space-y-3 text-sm">
                            <div className="flex justify-between">
                                ...
                                <span>₹{lineItems.reduce((sum, i) => sum + (i.price * i.quantity), 0).toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-gray-600">Add Discount</span>
                                <div className="flex items-center gap-2">
                                    <span>- ₹</span>
                                    <input type="number" value={discount} onChange={e => setDiscount(Number(e.target.value))} className="w-20 border rounded p-1 text-right" />
                                </div>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-gray-600">Add Shipping</span>
                                <div className="flex items-center gap-2">
                                    <span>+ ₹</span>
                                    <input type="number" value={shipping} onChange={e => setShipping(Number(e.target.value))} className="w-20 border rounded p-1 text-right" />
                                </div>
                            </div>
                            <div className="flex justify-between items-center">
                        ...
                        <div className="p-4 bg-gray-50 rounded-b-lg border-t border-gray-200">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <input type="checkbox" id="emailInvoice" checked={sendEmail} onChange={e => setSendEmail(e.target.checked)} className="h-4 w-4 text-blue-600" />
                                    <label htmlFor="emailInvoice" className="text-sm text-gray-700">Email invoice to customer</label>
                                </div>
                                <div className="flex gap-2">
                                    <button 
                                        onClick={() => setPaymentStatus(prev => prev === 'Paid' ? 'Pending' : 'Paid')}
                                        className={`px-4 py-2 rounded-md text-sm font-medium border ${paymentStatus === 'Paid' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-white text-gray-700 border-gray-300'}`}
                                    >
                                        {paymentStatus === 'Paid' ? 'Marked as Paid' : 'Mark as Paid'}
                                    </button>
                                    <button 
                                        onClick={handleSubmit} 
                                        disabled={loading}
                                        className="px-6 py-2 bg-gray-900 text-white rounded-md text-sm font-bold hover:bg-black disabled:opacity-50"
                                    >
                ...
                <div className="space-y-6">
                        ...
                        <div className="p-4">
                            <div className="relative mb-4" ref={customerSearchRef}>
                                ...
                                {isCustomerSearching && customerResults.length > 0 && (
                                    <div className="absolute z-50 w-full bg-white border border-gray-200 shadow-xl rounded-md mt-1 max-h-48 overflow-y-auto">
                                        {customerResults.map(u => (
                                            <div key={u.id} onClick={() => selectCustomer(u)} className="p-3 hover:bg-gray-50 cursor-pointer border-b last:border-0">
                            ...
                            {/* Customer Form (Editable even after selection) */}
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div className="p-4">
                            <textarea 
                                ...
                                className="w-full border-none resize-none text-sm text-gray-600 focus:ring-0 p-0"
                            ></textarea>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div className="p-4">
                            <input type="text" placeholder="Urgent, Reviewed, etc." className="w-full border border-gray-300 rounded p-2 text-sm" />
                        </div>
                    </div>
