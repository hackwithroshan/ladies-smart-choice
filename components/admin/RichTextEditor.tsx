...
const RichTextEditor: React.FC<RichTextEditorProps> = ({ value, onChange }) => {
    ...
    const ToolbarButton = ({ cmd, arg, icon, label, onClick, active }: { cmd?: string, arg?: string, icon?: React.ReactNode, label?: string, onClick?: () => void, active?: boolean }) => (
        <button
            ...
            className={`p-1.5 rounded transition-colors ${active ? 'bg-gray-200 text-black' : 'text-gray-600 hover:bg-gray-100'}`}
            title={label}
        >
            {icon}
        </button>
    ...
    return (
        <div className="border border-gray-300 rounded-lg overflow-hidden shadow-sm transition-all focus-within:ring-1 focus-within:ring-blue-500 focus-within:border-blue-500 bg-white">
                    ...
                    <>
                        ...
                        <div className="relative mr-2">
                            <select 
                                ...
                                className="h-8 text-xs border border-gray-300 rounded px-2 bg-white text-gray-700 focus:outline-none hover:bg-gray-50 cursor-pointer"
                                defaultValue="P"
                            >
            ...
            <div className="relative min-h-[300px]">
                ...
                {isUploading && (
                    <div className="absolute inset-0 z-10 bg-white/80 flex flex-col items-center justify-center">
                        ...
                        <span className="text-sm font-semibold text-blue-600">Uploading Image...</span>
                    </div>
                )}
             ...
             <div className="bg-gray-50 px-3 py-1 flex justify-between items-center text-[10px] text-gray-400 border-t">
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[80vh]">
                        <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50">
                            <h3 className="font-bold text-gray-800">Insert Image</h3>
                            <button onClick={() => setShowImageModal(false)} className="text-gray-400 hover:text-gray-600">&times;</button>
                        </div>
                        
                        <div className="flex border-b">
                            <button 
                                onClick={() => setImageTab('upload')}
                                className={`flex-1 py-3 text-sm font-medium transition-colors ${imageTab === 'upload' ? 'border-b-2 border-rose-600 text-rose-600 bg-rose-50' : 'text-gray-600 hover:bg-gray-50'}`}
                            >
                                Upload (Drag & Drop)
                            </button>
                            <button 
                                onClick={() => setImageTab('library')}
                                className={`flex-1 py-3 text-sm font-medium transition-colors ${imageTab === 'library' ? 'border-b-2 border-rose-600 text-rose-600 bg-rose-50' : 'text-gray-600 hover:bg-gray-50'}`}
                            >
                                Media Library
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50">
                            {imageTab === 'upload' ? (
                                <div 
                                    className={`h-64 border-2 border-dashed rounded-xl flex flex-col items-center justify-center text-center transition-all ${dragActive ? 'border-rose-500 bg-rose-50' : 'border-gray-300 bg-white'}`}
                                ...
                                >
                                    ...
                                    <button type="button" onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-white border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50 shadow-sm">Browse Files</button>
                                </div>
                            ) : (
                                <div className="h-full">
                                    {loadingLibrary ? <div className="flex justify-center items-center h-48"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-400"></div></div> : (
                                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                                            {libraryImages.length > 0 ? libraryImages.map((img) => (
                                                <div key={img.id} onClick={() => insertImage(img.url)} className="cursor-pointer group relative bg-white rounded-lg border border-gray-200 overflow-hidden aspect-square hover:ring-2 hover:ring-rose-500 hover:border-transparent">
                                                    <img src={img.url} className="w-full h-full object-cover" alt="Library" />
                                                </div>
                                            )) : <div className="col-span-full text-center py-10 text-gray-500">No images in library.</div>}
                                        </div>
                                    )}
                                </div>
                            )}
